#!/usr/bin/env python3
"""LlamaStash TUI UAT harness: drive the TUI in a pty, render with pyte, assert on screen text.

Reads a "program" (one step per line) from a file argument. Steps:
  spawn:<args>        (re)spawn llamastash with extra CLI args (space separated); optional
...
with `asciinema play <path>`, turn into a GIF with `agg <path> out.gif`). The
recording tees the raw PTY bytes the harness already reads, so it is exactly
what the app painted, driven by the scripted keystrokes. Use --cols/--rows to
drive a smaller terminal (handy for a tight README GIF); bracket the interesting
part with startcast/stopcast to skip load and quit.

The child inherits this process's env verbatim, so pair it with an isolated
state dir for a clean daemon: set LLAMASTASH_STATE_DIR / LLAMASTASH_CONFIG_DIR /
LLAMASTASH_CACHE_DIR / HF_HOME (see AGENTS.md). TERM is forced to xterm-256color.

Requires `pyte` and `pexpect` (pip install pyte pexpect — not in
scripts/requirements.txt because only this helper needs them; a venv is fine).
"""
import json
import os
import sys
...
# Match the canonical render size (llamastash --render-size / make render) so
# harness snapshots line up with the golden fixtures under tests/golden/.
COLS, ROWS = 160, 45
DEFAULT_SETTLE = 1.2

# Catppuccin Macchiato (the app's default theme) baked into the cast header so
# the shell / init-wizard ANSI colors render on-brand in asciinema-player and
# `agg` without extra flags. The TUI paints its own 24-bit colors regardless.
CAST_THEME = {
    "fg": "#cad3f5",
    "bg": "#24273a",
    "palette": ":".join([
        "#494d64", "#ed8796", "#a6da95", "#eed49f", "#8aadf4", "#f5bde6",
        "#8bd5ca", "#b8c0e0", "#5b6078", "#ed8796", "#a6da95", "#eed49f",
...
def default_bin():
    # scripts/tui/harness.py -> repo root is three levels up.
    ...
    def run(self, program):
        for raw in program:
            ...
            if op == "spawn":
                ...
                self.expect(arg, negate=True)
            elif op == "iexpect":
                self.expect(arg, ci=True)
            elif op == "comment":
                print(f"# {arg.strip()}")
            elif op == "startcast":
                self.start_cast()
...
def _take_opt(argv, name):
    ...
    Keeps the positional [binary args...] passthrough intact."""
    if name in argv:
        i = argv.index(name)
        value = argv[i + 1]
        del argv[i : i + 2]
...
def main():
    ...
    prog_file = argv[0]
    outdir = argv[1]
    binary = argv[2] if len(argv) > 2 else default_bin()
    base_args = argv[3:] if len(argv) > 3 else []
