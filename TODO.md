...
- [ ] Windows follow-ups deferred from 0.0.2 (post-0.0.2)
  ...
  - [ ] **Windows integration test triage.** Ten tests across four binaries are `#[cfg_attr(windows, ignore)]`. `tests/cli_integration_test.rs`: `agent_script_round_trip_list_start_status_logs_stop`, `start_preset_chain_seeds_supervisor_with_saved_params`, `start_ctx_above_native_succeeds_and_duplicate_launch_uses_new_port`, `logs_follow_returns_daemon_unreachable_when_daemon_dies`. `tests/proxy_eviction.rs`: `sweep_evicts_idle_auto_start_supervisor`, `sweep_skips_auto_start_with_inflight_request`. `tests/start_model_ipc_test.rs`: `start_model_drives_supervisor_status_logs_stop_and_last_params`, `prefer_port_falls_back_to_range_allocator_when_busy`, `last_params_persists_only_user_supplied_knob_deltas`. `tests/supervisor_ipc_test.rs`: `status_lists_active_supervised_model`. All hit the same root cause: `supervisor.stop()` on Windows falls through to the 5s grace + `TerminateJobObject` path because `GenerateConsoleCtrlEvent(CTRL_BREAK_EVENT, …)` can't reach a `CREATE_NO_WINDOW` child (they have different consoles). Brainstorm options: (a) spawn with a shared console flag so CTRL+BREAK reaches the child, (b) route shutdown through an HTTP IPC `/shutdown` endpoint instead of a console signal, (c) refactor the affected tests to tolerate the 5s grace ceiling. The sister tests in each binary pass on Windows after the Job Object singleton fix landed in 0.0.2, so the working baseline is good — these ten exercise multi-step start/stop chains or the eviction-stop path specifically.
