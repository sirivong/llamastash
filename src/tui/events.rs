...
/// One-off events generated by async tab tasks (embed, rerank). The
/// chat tab streams via its own `ChatStreamMsg` channel held inside
/// `ChatTabState`; embed/rerank only need a single result message
/// so the receivers live on their respective tab states.
#[derive(Debug)]
pub enum TabEvent {
...
pub fn spawn_logs_poller(
  ...
  tokio::spawn(async move {
    loop {
      let launch_id = current.lock().ok().and_then(|g| g.clone());
...
pub fn drain_embed_pending(app: &mut App) {
  let mut take = false;
  if let Some(rx) = app.embed.pending.as_mut() {
    match rx.try_recv() {
      Ok(TabEvent::EmbedOk(result)) => {
        app.embed.record(result);
        take = true;
      }
      Ok(TabEvent::EmbedErr(msg)) => {
        app.embed.record_error(msg);
        take = true;
      }
      Ok(_) => {} // Foreign variant — leave in queue.
...
pub fn drain_rerank_pending(app: &mut App) {
  let mut take = false;
  if let Some(rx) = app.rerank.pending.as_mut() {
    match rx.try_recv() {
      Ok(TabEvent::RerankOk(ranked)) => {
        app.rerank.record(ranked);
        take = true;
      }
      Ok(TabEvent::RerankErr(msg)) => {
        app.rerank.record_error(msg);
        take = true;
      }
      Ok(_) => {}
      Err(mpsc::error::TryRecvError::Empty) => {}
