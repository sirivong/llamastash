...
- Named launch presets now live in `config.yaml` (a `presets:` key) — the single writable source. `presets save` / `delete` (CLI) and the new TUI `Ctrl+P` save dialog write there comment-safely (only the touched entry changes; surrounding comments and formatting are preserved). A `presets:` key is per-model when it names a discovered model (basename, path fallback), otherwise an arch id applying to every model of that arch; a model's effective set is per-model ∪ arch (model wins), and an optional `default:` picks the entry the TUI's new preset cycle row opens on. The TUI Settings form gains a top-of-form preset cycle (`default → auto → named presets`) that re-seeds the knobs live. Existing `state.json` presets migrate into `config.yaml` once on upgrade. `status` model rows gain `preset_count` + `default`. See `config.example.yaml`.
