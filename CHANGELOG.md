...
- **BREAKING: per-backend `servers: []` arrays replace `binary` / `additional_binaries`.** Each backend's binary config is now a list of build/binary variants — `backend.llamacpp.servers: [{binary, name?}]` (first = default; each is a selectable "server"), and single-entry lists for `backend.ds4.servers` / `backend.lemonade.servers`. Every configured build is listed (no dedup across builds), so one install can offer CUDA / ROCm / Vulkan launches. New `start --server <id>` picks a build (and its backend); `status` gains a `servers` array (id, backend, binary, devices), and the model list surfaces `supported_backends` (all backends that can serve a model, e.g. `ds4|llamacpp`). No migration (pre-1.0) — update `config.yaml` (see `config.example.yaml`); `--llama-server` / `LLAMASTASH_LLAMA_SERVER` now set the first server. See `docs/plans/2026-07-16-001-feat-server-abstraction-plan.md`.
