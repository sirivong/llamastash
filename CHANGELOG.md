...
- LAN access for the OpenAI-compat proxy, opt-in and behind a bearer key. `llamastash daemon start --proxy-host 0.0.0.0` (or `proxy.host` / `LLAMASTASH_PROXY_HOST`, IPv6 supported) binds a routable address so other machines can reach your models. A non-loopback bind **requires** an API key: llamastash generates an `sk-llamastash-…` key on first use, saves it to `proxy.api_key` (atomic write, mode `0600`), prints it once, and requires `Authorization: Bearer <key>` on every data route (`/v1/*`, `/api/*`; `GET /` and `/health` stay open). The daemon refuses a non-loopback bind with no key unless you pass `--insecure-no-auth`. `status --json` reports the bound `host` + `auth` state (never the key), and the TUI flags an authed LAN listener. Only the proxy data plane is exposed — the control plane and `llama-server` children always stay loopback. TLS is not yet implemented, so LAN mode is plaintext (keep it on a trusted network or front it with a reverse proxy). (#25)
- Every typed launch knob is now a first-class `start` flag, not just a `-- <tail>` passthrough: `llamastash start qwen --threads 8 --device Vulkan0 --tensor-split 3,1 --flash-attn` works inline and is listed under **Advanced launch params** in `start --help`. The flags are generated from the same spec table the Settings editor reads, so the CLI and TUI surfaces can't drift — adding a knob yields a `start` flag and help line automatically. The trailing `-- <raw>` passthrough still carries llama-server single-dash shorts (`-ngl`) and arbitrary flags; a knob set on both surfaces wins from `--`. Inline knobs combined with `--preset` layer onto the preset rather than replacing it.
- Multi-GPU placement launch knobs → `--tensor-split` (`-ts`), `--main-gpu` (`-mg`), and `--split-mode` (`-sm`, `none|layer|row`). With these plus the existing `n_gpu_layers` / `n_cpu_moe` / `device` knobs, every GPU/CPU offload split llama-server supports is now settable as a typed knob (manually): dense partial-offload, MoE experts on CPU, and uneven splits across heterogeneous GPUs. Available everywhere the other typed knobs are (Settings editor, `start` CLI tail args, `arch_defaults` YAML, persisted `last_params`). The three placement knobs are hidden on single-GPU / CPU-only hosts.
- The Settings tab now groups the typed knobs into labelled clusters (Context, GPU / CPU offload, Multi-GPU placement, Attention & KV cache, Throughput, Memory loading, Advanced) ordered by how often they're changed, with related knobs kept together. Display order is independent of argv emission order.
