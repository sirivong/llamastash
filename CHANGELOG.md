...
- OpenAI-compatible proxy router on `127.0.0.1:11434` (enabled by default) — single URL, model-name routing across `/v1/models`, `/v1/chat/completions`, `/v1/completions`, `/v1/embeddings`, `/v1/rerank` with auto-start, single-flight launch coalescing, and family-MRU fallback on launch failure (substitution observable via `x-llamastash-served-by` + `x-llamastash-fallback-reason: launch_failed | family_mismatch` headers). Loopback only, no auth, no TLS. Surfaces through `status.proxy` (IPC + CLI `--json`) and the TUI Daemon info pane. Ships alongside the Ollama discovery surface — `GET /api/tags`, `GET /api/version`, `GET /api/ps`, `POST /api/show` — so Ollama-shape discovery libraries (`ollama-python` default path, `OLLAMA_HOST` env detection, IDE plugins that probe `/api/tags`) recognise llamastash and fall through to the OpenAI-compat endpoints for inference; Ollama's inference endpoints (`/api/chat`, `/api/generate`, `/api/embed`) are deferred (TODO §R2). `llamastash daemon start --proxy-port <PORT>` overrides `proxy.port` for one process (use `--proxy-port 0` for an ephemeral port; the actual address is reported via `llamastash status --json | jq .proxy.listen`). See [`docs/usage.md §Proxy (OpenAI-compatible listener)`](docs/usage.md#proxy-openai-compatible-listener) and [`docs/plans/2026-05-21-001-feat-proxy-router-plan.md`](docs/plans/2026-05-21-001-feat-proxy-router-plan.md).
