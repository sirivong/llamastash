...
- [x] ⚠️ **2.5** `.daemon.build == "0.0.2"`; `.server_path` points to the real Vulkan b9453 `llama-server.exe` (exists), but surfaces the Windows `\\?\` extended-length prefix [W-02].
...
- [x] ❌ **7.9** stale favorite **NOT filtered** [W-05]: favorited a temp `-p` model → 1; deleted the file (catalog → 0 models) + restarted daemon → `favorites list` **still returns 1** (dangling `\\?\…\temp-model.gguf`). Code review: `favorite_list` returns all persisted entries unfiltered and there is **no catalog/disk reconciliation anywhere** (CLI, daemon startup, or state load) — so this isn't Windows-specific, and pruning would be a behavior change (deferred for maintainer decision, not auto-fixed).
...
- [x] ⚠️ **12.3** Daemon panel: `port 48134`, `pid`, `up 0m`, `server …\b9453\llama-server.exe (rocm)`, `proxy listening 127.0.0.1:11435`, `models 5 found · 1 ready · 0 ★`, `running 1 (gemma-3-4b-it.Q5_K_M :41100)`. **Server tagged `(rocm)` though the installed binary is the Vulkan b9453 build** [W-07].
...
| W-02 | info | 2.5  | `.daemon.server_path` surfaces the Windows `\\?\` extended-length path prefix (`\\?\C:\Users\…\llama-server.exe`) in user-facing JSON. Cosmetic; functional path resolves.    | open   |
...
| W-05 | low  | 7.9  | **CLI `favorites list` does not filter stale entries; the TUI does.** `favorite_list` IPC returns all persisted favorites unfiltered, and `cli/favorites.rs` renders them verbatim — so a favorite whose file is deleted (catalog → 0) still appears. The TUI `info_pane::counts_row` *does* filter favorites against the catalog (`src/tui/info_pane.rs:202`). Aligning the CLI to filter like the TUI is a small fix but changes CLI output (would also hide favorites on temporarily-unmounted drives) → maintainer call. Not platform-specific. | needs-decision |
...
| W-07 | low  | 12.3 | **Server flavor mislabeled `(rocm)` for the Vulkan build.** `info_pane::flavor_label` derives the tag purely from GPU vendor (`GpuFlavor::Amd => "rocm"`, `src/tui/info_pane.rs:261`), but PR #11 now installs the **Vulkan** asset on Windows AMD. The installed variant isn't persisted (transient in `pick_asset_suffix`), so a correct fix needs either recording the variant at install or making `flavor_label` OS-aware (Windows AMD → vulkan, mirroring `gh_releases::pick_asset_suffix`). Loose end from the Windows-AMD→Vulkan fix. | needs-decision |
...
| W-09 | info | 13.7 | Hardware field names differ across surfaces: `init --json` emits `ram_bytes`/`vram_bytes`; `status --json .host` emits `ram_total_bytes`/`gpu_mem_total_bytes`. **Values are identical** (single codepath confirmed); only the key names diverge. Cosmetic contract inconsistency. | open (doc/cosmetic) |
...
| Items: ✅ / ❌ / ⚠️ / ⏭️     | **113 ✅ / 1 ❌ / 5 ⚠️ / 7 ⏭️** (126 total). ❌: 7.9 (W-05, stale-favorite filtering — not platform-specific). ⚠️: 1.10 (no cross-process SIGTERM), 2.5 (`\\?\` prefix), 5.10 (ctx-OOM on 8 GiB), 12.3 (`(rocm)` label on Vulkan build), 15.6 (orphan re-adoption N/A on Windows). ⏭️: 3.1/14.2/14.3 (no pty), 3.5/4.3 (no split fixture), 5.11 (no unknown-mode fixture), 9.9 (fallback engineering deferred). |
| Findings                     | 9 logged (W-01…W-09). **No functional regressions** — the core fixes all hold on Windows: host-panel **RAM exactly matches the OS** and `unified:false` shows **no `*` star** (original bug fixed); model **launches + reaches Ready on Vulkan** (launch crash fixed); single-codepath confirmed (`init` RAM == `status` RAM == OS); daemon lifecycle, presets/favorites, full proxy (OpenAI + Ollama + error envelopes + auto-start), `--render` matrix, init/doctor/pull all green. Findings are polish / platform-behavior items: 4 by-design (W-04/06/08 + the W-01 documented sampler delay), 2 cosmetic (W-02 `\\?\`, W-09 field names), 3 needing a maintainer decision (W-05 favorites filter, W-07 flavor label). |
