...
- **R81** First-push migration to `github.com/llamastash-rs/llamastash` with full URL cleanup. (Unit 1)
- **R82** Supporting repos: `homebrew-llamastash`, `llamastash-rs.github.io`, `.github` org profile. (Unit 1, 4, 5)
...
- **R90** `brew install llamastash-rs/llamastash/llamastash` + `--HEAD` source-build branch. (Unit 4)
...
- **What is the pre-tag guard for crate-name availability?** `curl -fsSL https://crates.io/api/v1/crates/<name>` returns 404 (unclaimed) or owner-list (must intersect llamastash-rs org members). 200 with non-member owner triggers the `llamastash-cli` rename fallback. (Origin R86 amended.)
...
│
├── llamastash-rs.github.io          ← marketing site (Units 5 + 6)
│   ├── src/                        ← Astro project
...
sequenceDiagram
    ...
    participant Site as llamastash-rs.github.io/bump.yml
...
```
brew install llamastash-rs/llamastash/llamastash
...
- [ ] **Unit 1: Repo migration + Cargo.toml + URL cleanup** *(partial — URL/metadata cleanup landed in `019fa78`. Org-admin actions documented in `docs/runbooks/release-0.0.1-bootstrap.md`; pending: create the four `llamastash-rs/*` GitHub repos and configure `CRATES_IO_TOKEN` + `GH_BUMP_TOKEN` secrets per the runbook)*

**Goal:** First-push the local working tree to `github.com/llamastash-rs/llamastash`, fix every URL reference, add missing Cargo metadata, create the empty supporting repos, prepare CHANGELOG.
...
**Files:**
- Modify: `Cargo.toml` (`repository`, `homepage`, `documentation` → `https://github.com/llamastash-rs/llamastash`; add `categories = ["command-line-utilities"]`; confirm `keywords` ≤ 5; review `exclude` list)
...
- Create: new GitHub repos (org-admin action, not source-controlled): `llamastash-rs/llamastash` (target of the push), `llamastash-rs/homebrew-llamastash` (empty, populated by Unit 4), `llamastash-rs/llamastash-rs.github.io` (empty, populated by Unit 5), `llamastash-rs/.github` (optional, with a one-paragraph `profile/README.md`)
- Test: none directly — verified by Unit 2's grep guard catching any remaining old-URL references.

**Approach:**
- The local tree currently has no `git remote`. Add `origin git@github.com:llamastash-rs/llamastash.git`, push `main`.
...
- Create `CRATES_IO_TOKEN` and `GH_BUMP_TOKEN` repo secrets before tagging — Unit 2's workflows are non-functional without them. `GH_BUMP_TOKEN` is a fine-grained PAT scoped to `contents:write` + `actions:write` on `homebrew-llamastash` and `llamastash-rs.github.io` only. (Upgrade to a scoped GitHub App is a 0.3.x consideration per origin §Outstanding Questions.)

**Patterns to follow:**
- `AGENTS.md` §"Docs stay in sync with code" — every file affected by the URL change is touched in the same commit.
- Conventional-commit prefix (per `AGENTS.md` §Conventions): commit subject `chore(release): migrate to llamastash-rs org` or similar; body lists the touched files.
...
**Verification:**
- The new `llamastash-rs/llamastash` repo on GitHub shows the project README rendered with no broken `github.com/llamastash/llamastash` links.
...
- Version resolution: if `LLAMASTASH_VERSION` env var or `--version <tag>` flag is set, use it; else `curl -fsSL https://api.github.com/repos/llamastash-rs/llamastash/releases/latest | jq -r '.tag_name'` — but jq is not always installed, so fall back to grep+sed on the JSON. (Bats fixture provides both jq-present and jq-absent cases.)
...
- A manual smoke run: against the test tag from Unit 2 (`v0.0.0-rc1`), `curl -fsSL https://github.com/llamastash-rs/llamastash/releases/download/v0.0.0-rc1/install.sh | sh` installs cleanly on a fresh Ubuntu container.
...
**Goal:** Stand up `llamastash-rs/homebrew-llamastash` with an initial binary formula and a `bump.yml` workflow that listens for `repository_dispatch` events from the main repo's release workflow and rewrites the formula on each new tag.

**Requirements:** R82, R89, R90.

**Dependencies:** Unit 1 (the tap repo must exist on GitHub). Develop-time independent of Units 2, 3, 5.

**Files (in `llamastash-rs/homebrew-llamastash` repo — not the main repo):**
...
- Test: no test files in the tap repo; verification is `brew install --force llamastash-rs/llamastash/llamastash` working against the test-tag.

**Approach:**
- Formula structure: `class Llamastash < Formula` with `desc`, `homepage`, `version`, `license`, `head "..." do ... end`, and four `if Hardware::CPU.arm? OS.mac?` (or `if OS.linux? && Hardware::CPU.intel?` etc.) branches setting `url` + `sha256` per target. The `head` branch fetches `github.com/llamastash-rs/llamastash.git` and builds from source with `system "cargo", "install", *std_cargo_args` so users opting into `--HEAD` get pre-release builds.
...
- Happy path: `brew install llamastash-rs/llamastash/llamastash` against a real tag fetches the tarball, verifies the SHA-256 declared in the formula, installs the binary to `Cellar`, links to `bin/llamastash`.
- Happy path: `brew install --HEAD llamastash-rs/llamastash/llamastash` clones the main repo and builds from source via cargo.
...
**Verification:**
- The tap repo exists, has the four files listed above, and `brew tap llamastash-rs/llamastash && brew search llamastash` returns a result.
- Against the test-tag from Unit 2: `brew install llamastash-rs/llamastash/llamastash` works on macOS and on Linux (with Linuxbrew).
...
- [ ] **Unit 5: Website scaffolding + Catppuccin theme + dispatch handler** *(scaffolded locally at `../llamastash-rs.github.io/`; `npm run build` clean, 440 KB dist; pending: org-admin creates the GitHub repo and first-pushes per `docs/runbooks/release-0.0.1-bootstrap.md`)*

**Goal:** Stand up `llamastash-rs/llamastash-rs.github.io` as an Astro 4 project with Tailwind + asciinema-player wired, the full Catppuccin Macchiato palette tokenized, the deploy workflow publishing to GitHub Pages, and the `bump.yml` handler ready to verify + commit `install.sh` from each new GH Release. **Site content (the actual page) lands in Unit 6; this unit is the chassis.**
...
**Files (in `llamastash-rs/llamastash-rs.github.io` repo):**
...
- Create: `public/install.sh` — placeholder containing only `#!/bin/sh\necho "no release yet — see github.com/llamastash-rs/llamastash"` until the first dispatch from Unit 2 overwrites it.
...
- Happy path: pushing to `main` triggers `deploy.yml`; the deploy succeeds within the 30s target; the deployed site renders at `llamastash-rs.github.io` (and at `llamastash.cli.rs` after Unit 7 lands DNS).
...
- Integration: after Unit 5 + Unit 6, the site URL `llamastash-rs.github.io` returns 200 and the hero install-tab labels match the version in `src/data/version.ts`.
...
- [ ] **Unit 6: Website content + asciinema cast + FAQ + accessibility** *(content authored in `../llamastash-rs.github.io/src/`; demo.cast + og-image + hero static are procedurally-generated placeholders pending real-recording exit criterion; Lighthouse pass deferred until live on Pages)*
...
**Test scenarios:**
- Happy path: visiting `llamastash-rs.github.io` (or `llamastash.cli.rs` after Unit 7) renders all eight sections in Macchiato palette.
...
- Pushing to `main` deploys; the live site at `llamastash-rs.github.io` shows all eight sections.
...
**Goal:** Land `llamastash.cli.rs → llamastash-rs.github.io` DNS via PR to the community-managed `cli.rs` zone-config repo, flip the site to serve under the branded URL, cut the actual `v0.2.0` tag, and smoke-test every install path on a fresh box. This is the public-launch unit.
...
- Modify: `llamastash-rs/llamastash-rs.github.io` repo's `public/CNAME` — already committed in Unit 5; this unit's role is to confirm it's still `llamastash.cli.rs` and that the file is in `dist/` after build.
...
- External: PR to the `cli.rs` zone-config repo adding a CNAME record `llamastash IN CNAME llamastash-rs.github.io.`.

**Approach:**
- Open the cli.rs PR first; while waiting for it to merge, the site is already live at `llamastash-rs.github.io` (Units 5+6). Marketing copy that mentions `llamastash.cli.rs` is held until DNS resolves.
- Once cli.rs DNS resolves: verify with `dig llamastash.cli.rs` returning a CNAME pointing at `llamastash-rs.github.io`. GitHub Pages auto-provisions an HTTPS cert via Let's Encrypt within a few minutes of CNAME working.
...
**Test scenarios:**
- Happy path: `dig llamastash.cli.rs` returns a CNAME to `llamastash-rs.github.io`; `curl https://llamastash.cli.rs` returns 200 with valid TLS.
...
- Happy path on fresh macOS: `brew install llamastash-rs/llamastash/llamastash` succeeds; binary runs.
...
- Edge case: cli.rs DNS takes hours/days to propagate after PR merge. Marketing copy that mentions `llamastash.cli.rs` is held; release proceeds with `llamastash-rs.github.io` as the temporarily-canonical URL; copy is amended once DNS is live.
