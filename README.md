...
- [Named presets, favorites, last-params recall](FEATURES.md#named-presets-favorites-last-params-recall) — presets live in `config.yaml` (per-model or per-arch), edited comment-safely from the CLI, the TUI `Ctrl+P` save dialog, or by hand; a preset cycle row in the Settings form picks one in a keystroke.
