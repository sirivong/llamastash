...
3. Config (`src/config/loader.rs`) — a typed config struct + a `Config` field, *only if* the backend has a `[<name>]` block. The YAML surface stays typed; the backend reads it through its own `available`/`installed` hooks; no consumer names it elsewhere.
